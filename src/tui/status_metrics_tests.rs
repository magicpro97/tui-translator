...
pub(crate) fn render_status_strip_with_stt(
    stt: SttState,
    width: u16,
    height: u16,
...
//
// WP-25.02 (#760): the audit noted "colour-only severity glyphs" but the
// real bug is the tests are too loose. `text.contains("INIT")` passes
// for "INITIAL", "INITIALIZING", "[INIT] ", etc. — none of those
// catch a regression that strips the `[`/`]` glyph. Tighten to require
// the exact bracket form, which is what the production code emits
// today and what a colour-blind / monochrome-terminal user relies on.
...
fn status_strip_renders_init_badge() -> Result<()> {
    ...
    assert!(
        text.contains("[INIT]"),
        "expected [INIT] badge with bracket glyph; got:\n{text}"
    );
    Ok(())
}
...
fn status_strip_renders_load_badge() -> Result<()> {
    ...
    assert!(
        text.contains("[LOAD"),
        "expected [LOAD... badge with bracket glyph; got:\n{text}"
    );
    Ok(())
}
...
fn status_strip_renders_ready_badge() -> Result<()> {
    ...
    assert!(
        text.contains("[READY]"),
        "expected [READY] badge with bracket glyph; got:\n{text}"
    );
    Ok(())
}
...
fn status_strip_renders_error_badge() -> Result<()> {
    ...
    assert!(
        text.contains("[ERROR]"),
        "expected [ERROR] badge with bracket glyph; got:\n{text}"
    );
    Ok(())
}
