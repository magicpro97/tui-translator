...
///
/// This runs **before** the TUI starts so download progress goes to stdout.
...
fn run_startup_stt_model_check(stdout: &mut impl io::Write) -> Result<()> {
    ...
    writeln!(
        stdout,
        "[tui-translator] Downloading Whisper {} (~{} MB) …",
        spec.id.display_name(),
...
fn run_startup_mt_model_check(stdout: &mut impl io::Write) -> Result<()> {
    ...
    // Presence check: encoder + decoder are the two large required files.
    ...
    writeln!(
        stdout,
        "[tui-translator] Downloading OPUS-MT ja→vi (~{total_mb} MB) …",
    )
    .ok();

    // Persist consent (auto-accept on first run; the user already chose local-only mode).
