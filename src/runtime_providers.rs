...
/// Resolve the local LLM model cache directory for the given optional override path.
///
/// When `override_path` is `None`, returns the platform-default model cache dir
/// joined with `llm/<model_id>`.
///
/// # Errors
/// Returns an error if the platform model cache dir cannot be determined.
#[allow(dead_code)] // used inside #[cfg(feature = "local-llm-mt")] block
